...
//				int bidIncrement, String category)
		...
		itemList.add(
				new Item("Glow Stick", "THE BEST STICK IN THE WORLD!", 10.0, today, null, 1, "Light utilities", 1));
	...
	public static String loginChecker(ArrayList<UserAccount> loginList, ArrayList<UserAccount> currentUser,
			String email, String password, String status, String role) {
		...
		for (UserAccount us : loginList) {
			...
			} else if (email.equals(us.getEmail()) && password.equals(us.getPassword())
					&& (us.getStatus().equals("blocked"))) {
				status = "blocked";
			} else if (email.equals(us.getEmail()) && password.equals(us.getPassword())
					&& (us.getStatus().equals("invalid"))) {
				status = "invalid";
			}
		}
		return status;
...
//						int bidIncrement, String category, int itemId) {
				...
				if (foundItem == false) {
					...
					itemList.add(new Item(itemAdd, description, minimumBidPrice, today, endDate, bidIncrement, category,
							lastNumber++));
	...
	public static void bidItem(ArrayList<Item> itemList, ArrayList<UserAccount> currentUser) {
		...
		for (int i = 0; i < itemList.size(); i++) {
			...
			for (int j = 0; j < itemArray.length; j++) {
				if (search.strip().equalsIgnoreCase(itemArray[j].strip())
						|| search.equalsIgnoreCase(item.getItemName())) {
					System.out.println("Item Name: " + item.getItemName() + "\nDescription: " + item.getDescription()
							+ "\nCategory: " + item.getCategory() + "\nMinimum Bid Price: $" + item.getMinimumBidPrice()
							+ "\nBid Increment: $" + item.getBidIncrement() + "\nAuction Start Date: "
							+ item.getAuctionStart());
